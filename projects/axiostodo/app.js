...
function deleteItem(id) {
    axios.delete(`https://api.vschool.io/chazz/todo/${id}`);
...
function markToDoCompleted(itemId) {
    ...
    if (checked.className === 'done') {
        axios.put(`https://api.vschool.io/chazz/todo/${itemId}`, {completed: false} )
        checked.className = '';
    } else {
        axios.put(`https://api.vschool.io/chazz/todo/${itemId}`, {completed: true} )
        checked.setAttribute("class", "done")
    }
    // console.log(checked.className)
}

const apiList = axios.get("https://api.vschool.io/chazz/todo/").then(function (response) {
...
localForm.addEventListener("submit", function () {
    ...
    axios.post("https://api.vschool.io/chazz/todo/", newToDoItem).then(function (response) {
...
deleteButton.addEventListener("click", function () {
    ...
    inputs.forEach(input => {
        if (input.checked) {
            ...
            // console.log(x)
            axios.delete(`https://api.vschool.io/chazz/todo/${x}`)
